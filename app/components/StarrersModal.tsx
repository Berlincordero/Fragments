...
export default function StarrersModal({
  visible,
  postId,
  onClose,
}: {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await fetch(endpoints.fincaPostStarrers(postId), {
          headers: { Authorization: `Token ${tk}` },
        });
        const data = await res.json();
        setList(Array.isArray(data.results) ? data.results : []);
      } catch {
        Alert.alert("Error", "No se pudo cargar las reacciones");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <Modal animationType="slide" visible={visible} onRequestClose={onClose}>
      <SafeAreaView style={mStyles.modalSafe}>
        <View style={mStyles.modalHeader}>
          ...
          <Text style={mStyles.headerTitle}>Personas que dieron una ⭐</Text>
          <View style={{ width: 92 }} />
        </View>
            ...
            {list.map((u) => (
              <View key={u.username} style={mStyles.row}>
                ...
                <Text style={mStyles.rowText}>
                  <Text style={{ fontWeight: "700" }}>{u.display_name}</Text> te dio una estrella
                </Text>
              </View>
            ))}
            {!list.length && (
              <Text style={mStyles.empty}>Aún no hay estrellas</Text>
            )}
          </ScrollView>
        )}
      </SafeAreaView>
