...
export default function RepostersModal({
  visible,
  postId,
  onClose,
}: {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await fetch(endpoints.fincaPostReposters(postId), {
          headers: { Authorization: `Token ${tk}` },
        });
        const data = await res.json();
        setList(Array.isArray(data.results) ? data.results : []);
      } catch {
        Alert.alert("Error", "No se pudo cargar la lista de compartidos");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <Modal animationType="slide" visible={visible} onRequestClose={onClose}>
      <SafeAreaView style={mStyles.modalSafe}>
        <View style={mStyles.modalHeader}>
          ...
          <Text style={mStyles.headerTitle}>Usuarios que compartieron esta publicación</Text>
          <View style={{ width: 92 }} />
        </View>
